...
npm run apkg -- --in input.xlsx --out vocab.apkg --deck "我的词汇" --tts youdao --exclude exclude.csv --verbose
```




请帮我处理PDF文件，严格按照以下要求输出，无需多余内容，直接满足可复制、易粘贴到Excel的需求：

1. 提取PDF中的序号、单词、音标、释义、例句，整理为表格格式，表格需保持Excel格式，带一键复制按钮，点击复制按钮可一键复制整个表格内容（直接适配Excel粘贴，无格式错乱）；

2. 表格列名及内容要求：
- 第一列：序号，**直接提取PDF里原有的序号**，不重新生成；
- 第二列：单词（含音标），保留原文单词和对应音标，格式与PDF一致；
- 第三列：释义，完整保留原文释义（中英文均保留，无遗漏）；
- 第四列：例句，完整保留原文例句（中英文均保留，标点、格式不变）；
- 表格按PDF中单词的原有顺序排列，纯文本对齐，无多余空格、无乱码。

3. 额外任务：单独提取纯单词（去掉所有音标），同样放入【带一键复制按钮的文本框】中，每个单词单独一行，依次排列，点击复制按钮可一键复制所有纯单词。

4. 整体输出要求：
- 仅输出一个表格、一个纯单词列表，每个都必须带有【一键复制】按钮；
- 无任何链接、无附件、无多余说明文字；
- 内容可直接复制，粘贴到Excel中无格式错乱，纯单词列表粘贴后自动分行。
